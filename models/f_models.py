...
class Models():
    ...
    def iterate_models(self):
        ...
        dados_x = self.data_clean(dados, remove_target_list, 'X')
        dados_y_all = self.data_clean(dados, remove_target_list, 'Y')
        ...
        for target_eval in target_list_bol:
            ...
            X_train, y_train = self.balance_sample(X_train, y_train, 2)
            ...
            # self.create_model(LogisticRegression(class_weight='balanced',random_state=0,max_iter=1000), 'logistic_regression', version_model, root_path, target_eval, X_train_norm, y_train, X_test_norm, y_test)
            self.create_model(SVC(probability=True, kernel='linear', C=0.7, max_iter=1000), 'SVC', version_model, root_path, target_eval, X_train_norm, y_train, X_test_norm, y_test)
            #  SVC(kernel='rbf', C=1.0, gamma='scale', max_iter=1000))
            # Não necessitam de dados normalizados
            # self.create_model(RandomForestClassifier(), 'random_forest', version_model, root_path, target_eval, X_train, y_train, X_test, y_test)
            # self.create_model(XGBClassifier(), 'XGB', version_model, root_path, target_eval, X_train, y_train, X_test, y_test)
